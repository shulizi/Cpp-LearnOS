...
StateStruct::StateStruct(int boatSite){
	this->boatSite = boatSite;
    this->objectClassesNumber = 0;
}
StateStruct::StateStruct(StateStruct* stateStruct){
    this->boatSite = stateStruct->boatSite;
...
void StateStruct::boating(int numbers[]){
    if(this->boatSite == 1){//1代表船从左往右，0代表船从右往左
        for(int i=0;i<this->objectClassesNumber;i++){
    ...
    this->boatSite = 1 - this->boatSite;
}
bool StateStruct::isEqual(StateStruct* state,int a[]){
    if(this->boatSite != 1 - state->boatSite){
        return false;
    }
   
    
    for(int i=0;i<objectClassesNumber;i++){
        if(this->objectStructs[i]->leftBankNumber != state->objectStructs[i]->leftBankNumber - a[i]* (state->boatSite*2-1)){//(state->boatSite*2-1)表示船左(boatSite = 1)减(leftBankNumber - a[i])右(boatSite = 0)加(leftBankNumber + a[i])
            return false;
        }
    }
...
bool StateStruct::isEqual(StateStruct* state){
    if(this->boatSite !=  state->boatSite){
        return false;
    }
    for(int i=0;i<objectClassesNumber;i++){
